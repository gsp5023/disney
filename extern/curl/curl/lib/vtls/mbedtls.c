...
#if defined(USE_SB_SOCKET)
static int send_sb_socket(void * ctx, const unsigned char * buf, size_t len)
{
  sb_socket_t* fd = ((sb_socket_t *)ctx);
  int bytes_sent;

  const_mem_region_t message = CONST_MEM_REGION(buf, len);

  sb_socket_send_result_t send_errors = sb_socket_send(*fd, message, 0, &bytes_sent);

  if (send_errors.result == sb_socket_send_success) {
    return bytes_sent;
  }

  switch (send_errors.result)
  {
...
static int recv_sb_socket(void * ctx, unsigned char * buf, size_t buffersize)
{
  sb_socket_t * fd = ((sb_socket_t *)ctx);
  int bytes_recvd;

  sb_socket_receive_result_t recv_errors = sb_socket_receive(*fd, MEM_REGION(buf, buffersize), 0, &bytes_recvd);

  if (recv_errors.result == sb_socket_receive_success) {
    return bytes_recvd;
  }

  return recv_errors.result == sb_socket_receive_would_block ? MBEDTLS_ERR_SSL_WANT_READ : MBEDTLS_ERR_NET_RECV_FAILED;
}
#endif
