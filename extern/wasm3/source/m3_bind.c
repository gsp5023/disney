...
_   (SignatureToFuncType (& ftype, i_linkingSignature));
    ...
    {
        debug_write_line("got: 0x%08x, expected: 0x%08x\n", i_function->funcType, ftype);
        debug_write_line(
            "ERROR: invalid signature for %s: expected: %s - found: %s",
            GetFunctionName(i_function),
            SPrintFuncTypeSignature(ftype),
