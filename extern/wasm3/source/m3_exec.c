...
d_m3OpDef  (Call)
{
    ...
    m3ret_t r = Call (callPC, sp, _mem, d_m3OpDefaultArgs, ctx);
    ...
    if (tableIndex < module->table0Size)
    {
        ...
        if (function)
        {
            if (type == function->funcType)
            {
                ...
                if (not r)
                {
                    r = Call (function->compiled, sp, _mem, d_m3OpDefaultArgs, ctx);
...
d_m3OpDef  (CallRawFunction)
{
    ...
    _mem->runtime->stackPointerAtLastRawFunctionCall = _sp;
...
d_m3OpDef  (MemGrow)
{
    ...
    if (numPagesToGrow)
    {
        ...
        if (r) {
            _r0 = -1;
            return m3_StoreErrorAndStackTrace(runtime, r);
        }
...
#endif
    {
        m3_exec_ctx_push_call(ctx, function);
...
#       endif
        ...
        m3_exec_ctx_pop_call(ctx);

        return r;
    }

    return m3_StoreErrorAndStackTrace(_mem->runtime, m3Err_trapStackOverflow);
}
