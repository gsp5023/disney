    ...
    if (numRequiredSlots == 2 and numUsedConstSlots >= 2)
    {
        ...
        for (int slot = firstConstSlot; slot < o->maxConstSlotIndex - 1; slot += 2)
        {
            if (IsSlotAllocated (o, slot) and IsSlotAllocated (o, slot + 1))
            {
                ...
                if (is64BitType) {
                    ex.to_m3slot = & o->constants [slot - o->firstConstSlotIndex];
                    constant = * ex.to_u64;
                } else {
                    ex.to_m3slot = & o->constants [slot - o->firstConstSlotIndex];
                    constant = * ex.to_u32;
...
_   (ReadLEB_u32 (& functionIndex, & o->wasm, o->wasmEnd));
    ...
    if (function)
    {
        m3log (compile, d_indent "%s (func= '%s'; args= %d)", get_indention_string (o), GetFunctionName (function), function->funcType->numArgs);
...
_           (EmitOp     (o, op));
        ...
        else
        {
            result = ErrorCompile (
                ...
                functionIndex,
                o->module->numFunctions);

            debug_write_line("ERROR: %s %s\n", m3Err_functionImportMissing, o->runtime->error_message);
        }
    }
