...
//  Copyright (c) 2019 Volodymyr Shymanskyy. All rights reserved.
//
...
#define m3ErrReport_trapDivisionByZero m3_StoreErrorAndStackTrace(_mem->runtime, m3Err_trapDivisionByZero)
#define m3ErrReport_trapIntegerOverflow m3_StoreErrorAndStackTrace(_mem->runtime, m3Err_trapIntegerOverflow)
#define m3ErrReport_trapIntegerConversion m3_StoreErrorAndStackTrace(_mem->runtime, m3Err_trapIntegerConversion)

#define OP_DIV_U(RES, A, B)                                     \
    if (UNLIKELY(B == 0)) return m3ErrReport_trapDivisionByZero;\
    RES = A / B;
...
// 2's complement detection
    ...
    #define OP_DIV_S(RES, A, B, TYPE_MIN)                           \
        ...
        if (UNLIKELY(B == -1 and A == TYPE_MIN)) {                  \
            return m3ErrReport_trapIntegerOverflow;                 \
        }                                                           \
        RES = A / B;
...
#define OP_TRUNC(RES, A, TYPE, RMIN, RMAX)                  \
    if (UNLIKELY(isnan(A))) {                               \
        return m3ErrReport_trapIntegerConversion;           \
    }                                                       \
