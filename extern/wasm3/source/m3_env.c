...
M3Result  EvaluateExpression  (IM3Module i_module, void * o_expressed, u8 i_type, bytes_t * io_bytes, cbytes_t i_end)
{
    ...
    if (o->page)
    {
        ...
        {
            m3_exec_ctx ctx = {0};
            m3ret_t r = Call (m3code, stack, NULL, d_m3OpDefaultArgs, &ctx);
            result = runtime.runtimeError;

            if (r == 0 and not result)
            {
                M3PointerExchange ex;
                if (SizeOfType (i_type) == sizeof (u32))
                {
                    ex.to_m3slot = stack;
                    *(u32 *)o_expressed = * ex.to_u32;
                }
                else
                {
                    ex.to_m3slot = stack;
                    *(u64 *)o_expressed = * ex.to_u64;
                }
            }
        }
...
# endif
    ...
    else result = m3_StoreErrorAndStackTrace(io_runtime, m3Err_wasmMemoryOverflow);
...
_           (ReadLEB_u32 (& numElements, & bytes, end));
            ...
            if ((i64)endElement > (i64)offset) // TODO: check this, endElement depends on offset
            {
...
M3Result  m3_CallWithArgs  (IM3Function i_function, uint32_t i_argc, const char * const * i_argv)
{
    ...
    if (i_function->compiled)
    {
        ...
        m3_exec_ctx ctx = {0};
_       ((M3Result) Call (i_function->compiled, (m3stack_t) stack, runtime->memory.mallocated, d_m3OpDefaultArgs, &ctx));
