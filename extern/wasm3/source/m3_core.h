...
typedef struct M3MemoryHeader
{
    ...
    void *          maxStack;
    size_t          length;
    size_t          padd; // fix m3 not aligning 8 byte types
}
M3MemoryHeader;

static const int STATIC_ASSERT_M3MEMORY_HEADER_IS_8BYTE_ALIGNED[((sizeof(M3MemoryHeader) & 0x7) == 0)] = { 0 };
...
typedef union M3PointerExchange
{
    void * *                to_ptr;
    m3slot_t *              to_m3slot;
    u32 *                   to_u32;
