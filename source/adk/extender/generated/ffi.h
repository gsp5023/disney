...
typedef struct adk_native_functions_t {
    extensions_ffi_e ffi_table_hash;
    ...
    sb_file_t * (*sb_fopen)(const sb_file_directory_e directory, const char *const path, const char *const mode);
    _Bool (*sb_fclose)(sb_file_t *const file);
    _Bool (*sb_delete_file)(const sb_file_directory_e directory, const char *const filename);
    sb_directory_delete_error_e (*sb_delete_directory)(const sb_file_directory_e directory, const char *const subpath);
    _Bool (*sb_create_directory_path)(const sb_file_directory_e directory, const char *const input_path);
    ...
    _Bool (*sb_fseek)(sb_file_t *const file, const long offset, const sb_seek_mode_e origin);
    size_t (*sb_fread)(void *const buffer, const size_t elem_size, const size_t elem_count, sb_file_t *const file);
    size_t (*sb_fwrite)(const void *const buffer, const size_t elem_size, const size_t elem_count, sb_file_t *const file);
    _Bool (*sb_feof)(sb_file_t *const file);
    _Bool (*sb_rename)(const sb_file_directory_e mount_point, const char *const current_path, const char *const new_path);
    bundle_t * (*bundle_open)(const sb_file_directory_e directory, const char *const path);
    _Bool (*bundle_close)(bundle_t *const bundle);
    ...
    sb_thread_id_t (*sb_create_thread)(const char *name, const sb_thread_options_t options, int (*const thread_proc)(void *), void *const arg, const char *const tag);
    ...
    void (*sb_destroy_condition_variable)(sb_condition_variable_t *const cnd, const char *const tag);
    void (*sb_condition_wake_one)(sb_condition_variable_t *cnd);
    void (*sb_condition_wake_all)(sb_condition_variable_t *cnd);
    _Bool (*sb_wait_condition)(sb_condition_variable_t *cnd, sb_mutex_t *mutex, const milliseconds_t timeout);
    ...
    void (*cncbus_init_receiver)(cncbus_receiver_t *const receiver, const struct cncbus_receiver_vtable_t *const vtable, const cncbus_address_t address);
    ...
    _Bool (*cncbus_msg_reserve_unchecked)(cncbus_msg_t *const msg, const int size);
    _Bool (*cncbus_msg_write_unchecked)(cncbus_msg_t *const msg, const void *const src, const int size);
    _Bool (*cncbus_msg_set_size_unchecked)(cncbus_msg_t *const msg, int size);
    ...
    void (*sb_get_system_metrics)(struct adk_system_metrics_t *const out);
    void (*sb_tick)(const struct adk_event_t **const head, const struct adk_event_t **const tail);
    ...
    void (*adk_app_metrics_report)(const char *const app_id, const char *const app_name, const char *const app_version);
    ...
    _Bool (*sb_enable_blocking_socket)(const sb_socket_t sock, const sb_socket_blocking_e blocking_mode);
    _Bool (*sb_socket_set_tcp_no_delay)(const sb_socket_t sock, const sb_socket_tcp_delay_mode_e tcp_delay_mode);
    _Bool (*sb_socket_set_linger)(const sb_socket_t sock, const sb_socket_linger_mode_e linger_mode, const int linger_timeout_in_seconds);
